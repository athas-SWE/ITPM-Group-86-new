...
import img2 from "./image/NurProfile.png";
...
const NurDetails = () => {
  ...
  const getdata = async () => {
    const res = await fetch(`http://localhost:5000/Nurse/getuser/${id}`, {
      method: "GET",
      headers: {
  ...
  const genPDF = () => {
    ...
    doc.setFontSize(20);
    doc.text("SYS Acdemy ", 10, 10);

    doc.autoTable({
      html: "#content",
    });
    doc.save("Parent Details.pdf");
  };
  ...
  return (
    <div className="container mt-3">
      ...
      <div className="add_btn_2 mt-2 mb-20">
        <NavLink to={`/NurseEdit/${getuserdata._id}`}>
      ...
      <div className="row mt-3">
        <table className="table" id="content">
          ...
          <tbody>
            <tr>
              <th scope="row">{getuserdata.nurseID}</th>
        ...
        <div className="left_view col-lg-4 col-md-4 col-8">
          <h3>
            Parent No: <span>{getuserdata.nurseID}</span>
          </h3>
          <h3>
        ...
        <div className="right_view col-lg-4 col-md-4 col-8">
          ...
          <h3>
            <i class="fa-solid fa-house-medical"></i> Address:{" "}
